    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .fill(isFlipped ? Color(red: 0.17, green: 0.68, blue: 0.93) : Color(.systemBackground))
                .shadow(color: Color.black.opacity(0.15), radius: 16, x: 0, y: 10)

            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .strokeBorder(isFlipped ? Color.white.opacity(0.6) : Color.secondary.opacity(0.15), lineWidth: 1)

            VStack(spacing: 20) {
                if isFlipped {
                    // Back: definition + antonyms
                    VStack(spacing: 10) {
                        Text(item.word)
                    ...
                    // Front: word + audio button + hint text
                    VStack(spacing: 16) {
                        Text(item.word)
